...
class UserInfo(models.Model):
    ...
    exchangeCode = models.CharField(
        verbose_name="取引所コード", max_length=10, choices=[e.value for e in TradeCode]
    )
    def __str__(self):
          return self.userId
...
class TradeManage(models.Model):
    ...
    coinType = models.CharField(
        verbose_name="通貨種別", max_length=10, choices=[e.value for e in CoinType]
    )
    tradeStatus = models.CharField(
        verbose_name="取引ステータス", max_length=10, choices=[e.value for e in TradeStatus]
    )
    orderDate = models.CharField(verbose_name="注文日", max_length=8)
    orderTime = models.CharField(verbose_name="注文時刻", max_length=12)
    finishDate = models.CharField(verbose_name="完了日", max_length=8, default=None, null=True)
    finishTime = models.CharField(verbose_name="完了時刻", max_length=12, default=None, null=True)
    amount = models.FloatField(verbose_name="単位数")
    orderPrice = models.FloatField(verbose_name="注文価格")
    contractPrice = models.FloatField(verbose_name="約定価格")
    def __str__(self):
          return self.tradeMgtNo.__str__
